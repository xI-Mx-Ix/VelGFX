...
import net.xmx.velgfx.renderer.mesh.standalone.VxStandaloneMesh;
import net.xmx.velgfx.renderer.mesh.arena.VxArenaBuffer;
import net.xmx.velgfx.renderer.mesh.arena.VxArenaMesh;
...
public final class VxModelManager {
    ...
    public static Optional<VxStandaloneMesh> getStandaloneMesh(ResourceLocation location) {
        ...
        if (definitionOpt.isPresent()) {
            ...
            VelGFX.LOGGER.error("Failed to create standalone mesh because definition failed to load: {}", location);
    ...
    public static Optional<VxArenaMesh> getArenaMesh(ResourceLocation location) {
        ...
        if (definitionOpt.isPresent()) {
            ...
            if (subMesh != null) {
                ...
                VelGFX.LOGGER.error("Failed to allocate arena sub-mesh for: {}", location);
                return Optional.empty();
            }
        } else {
            VelGFX.LOGGER.error("Failed to create arena sub-mesh because definition failed to load: {}", location);
            return Optional.empty();
        }
    }
    ...
    private static Optional<VxMeshDefinition> getDefinition(ResourceLocation location) {
        ...
        try {
            ...
            VelGFX.LOGGER.error("Failed to parse model definition: {}", location, e);
            return Optional.empty();
        }
    }
